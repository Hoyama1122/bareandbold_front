...
const INITIAL_UAT_ITEMS = [
  // Section 1: Customer Features
  {
    ...
    expected: "สร้างบัญชีสำเร็จ บันทึกลงฐานข้อมูล PostgreSQL พร้อมเข้ารหัสรหัสผ่าน",
    category: "customer",
    passed: true
  },
  {
    ...
    expected: "เข้าสู่ระบบสำเร็จ ระบบส่ง JWT Token เก็บลง HTTP-only Cookie ของ Browser",
    category: "customer",
    passed: true
  },
  {
    ...
    expected: "แสดงตัวอย่างภาพหินมงคล/สายรัด คำนวณราคาเพิ่มขึ้นหรือลดลงอย่างถูกต้อง",
    category: "customer",
    passed: false
  },
  {
    ...
    expected: "สินค้าปรากฏในตะกร้า ข้อมูลสเปกคัสตอมถูกบันทึก และสามารถปรับจำนวนชิ้นได้จริง",
    category: "customer",
    passed: true
  },
  {
    ...
    expected: "สร้างรหัสใบสั่งซื้อออเดอร์ในสถานะ PENDING พร้อมทำรายการชำระสำเร็จจะเปลี่ยนสถานะอัตโนมัติ",
    category: "customer",
    passed: true
  },
  {
    ...
    expected: "แสดงรายการออเดอร์ในสถานะปัจจุบัน เช่น PAID หรือ SHIPPED พร้อมเลขติดตามพัสดุ",
    category: "customer",
    passed: true
  },
  ...
  },
  {
    ...
    expected: "วัสดุในระบบสต็อกคัสตอมอัปเดตยอดคงคลังและราคาให้ลูกค้าออกแบบได้ถูกต้อง",
    category: "staff",
    passed: false
  },
  {
    ...
    expected: "สถานะออเดอร์เปลี่ยน และรหัสขนส่งถูกบันทึกส่งตรงไปแสดงฝั่งลูกค้าผู้ซื้อ",
    category: "staff",
    passed: true
  },
  ...
  },
  {
    ...
    expected: "แสดงกราฟ/ยอดรวมรายได้ของสินค้าพร้อมส่ง และสินค้าแบบคัสตอมแยกหมวดหมู่ชัดเจน",
    category: "manager",
    passed: false
  },
  ...
  // Section 4: Infrastructure
  {
    ...
    title: "12. ระบบอัปโหลดรูปภาพใบเสร็จและสินค้า (Cloudflare R2 Object Storage)",
    step: "อัปโหลดภาพประกอบสินค้า หรืออัปโหลดรูปสลิปตอนแจ้งโอนผ่านหน้าเว็บ",
    expected: "ไฟล์รูปภาพถูกจัดเก็บแบบไร้รอยต่อบนคลาวด์ R2 และดึงแสดงผลได้เสถียรปลอดภัย",
    category: "infra",
    passed: true
  }
];
...
export default function UatPage() {
  ...
  const passedCount = INITIAL_UAT_ITEMS.filter(item => !!passedItems[item.id]).length;
          ...
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
            <div>
              <h1 className="text-3xl md:text-4xl lg:text-5xl font-black uppercase tracking-tight leading-none text-[#3C322A]">
                USER ACCEPTANCE<br />TESTING (UAT)
              </h1>
              <p className="text-xs font-bold text-[#6A5242] mt-3 max-w-lg leading-relaxed">
                รายการแบบตรวจสอบประเมินผลการทำงานจริงของระบบ (UAT Checklist) โดยค่าตั้งต้นจะอ้างอิงจากรหัส `passed: true/false` ในโค้ด เมื่อทำการ Git Push ลง Main จะแสดงผลตามนั้น
              </p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
              ...
              <span className="hidden sm:inline text-sm font-bold text-[#6A5242]/70">UAT v1.1</span>
            </div>
          </div>
        </div>
      </section>
          ...
          <div className="bg-white border border-[#EADECC]/60 rounded-2xl p-6 md:p-8 shadow-xs flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="space-y-2.5 flex-1 w-full">
              <div className="flex justify-between items-end">
                <span className="text-xs font-black uppercase text-[#6A5242] tracking-wider">ภาพรวมผลการทดสอบความถูกต้อง (UAT Completion Rate)</span>
                <span className="text-2xl font-black text-[#3C322A]">{percentComplete}%</span>
              </div>
              ...
              <div className="flex justify-between text-xs font-semibold text-[#6A5242]">
                <span>ผ่านการทดสอบแล้ว {passedCount} จาก {totalCount} รายการ</span>
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 border-b border-[#EADECC]/45 pb-4">
            <div className="flex gap-1.5">
              {[
                ...
                { id: "pending", label: `รอดำเนินการ (${totalCount - passedCount})` }
              ].map(filter => (
                <button
                  key={filter.id}
          ...
          <div className="space-y-10">
            {filteredItems.length === 0 ? (
              <div className="text-center py-20 border border-dashed border-[#EADECC] rounded-2xl bg-white/40">
                <span className="text-sm font-bold text-[#6A5242]/50 italic">ไม่พบการทดสอบ UAT ตามที่ระบุฟิลเตอร์ไว้</span>
              </div>
            ) : (
                          ...
                          >
                            <div className="space-y-1.5 flex-1 min-w-0">
                              <div className="flex items-center gap-2">
                                <span className={`w-2 h-2 rounded-full ${isPassed ? "bg-emerald-500" : "bg-amber-400"}`} />
                                <h4 className="text-sm font-black text-[#3C322A]">{item.title}</h4>
                              </div>
                              <div className="pl-4 space-y-1 text-xs text-[#6A5242] font-semibold leading-relaxed">
                                <p><strong className="font-extrabold text-[#3C322A]">ขั้นตอนทดสอบ:</strong> {item.step}</p>
                                <p><strong className="font-extrabold text-[#3C322A]">ผลลัพธ์ที่คาดหวัง:</strong> {item.expected}</p>
                              </div>
                            </div>
